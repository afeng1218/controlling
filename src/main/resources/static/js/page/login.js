        ...
        function checkLogin() {
            ...
            if (name == '' || password == '') {
                ...
                if (password == '' && name != '') {
                     ...
                     * 验证用户是否需要密码登陆
                     */
                    COMMON.WS.local('login/checkNeedPassword', 'get', {name: name}, false, function (data) {
                        if (data.result + '' == 'true') {
            ...
            COMMON.WS.local('login/checkUser', 'get', uploadValue, true, function (data) {
                ...
                $.cookie('orderApprovalAuthority', orderApprovalAuthorityValue);
                /**
                 * 获取用户页面权限
                 */
                ...
                if (pageAuthority != null) {
                    for (var i = 0; i < pageAuthority.length; i++) {
                        $.cookie(COMMON.ECODE.Base64.encode(pageAuthority[i].linkPage),
                            COMMON.ECODE.Base64.encode(pageAuthority[i].authority));
                    }
                }
                /**
                 * 跳转到主页面
